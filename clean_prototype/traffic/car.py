...
class Car:
    def __init__(self, path:list[Way]):
        ...
        self.car_dist:float = 1
        self.accel:float = 15.0
    ...
    def _turn_lookahead(self):
        max_dist = self.target_speed * 2
        ...
        return 1 / (1 + curvature * 0.25)

    def car_lookahead(self):
        ...
        if next_car:
            return max(0, min(1, (lookahead - self.car_dist * max(10, self.speed)) / max(0.001, lookahead)))# ** (1 / (3 * max(next_car.speed, 0.001)))
        return 1
    ...
    def update(self, dt:float) -> None:
        ...
        turn_multiplier = self._turn_lookahead()
        car_multiplier = self.car_lookahead()
        state = self._set_speed(dt, min(turn_multiplier, car_multiplier))
